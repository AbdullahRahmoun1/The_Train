...
{
public:
	static bool swapMoving;
	static double RightHand,LeftHand,RightLeg,leftLeg,value;
	Player(void);
	static void drawAndManage();
	~Player(void);
	class Board
{
		public:
			...
			void build()
			{
				...
				glPushMatrix();
					...
					*/
					if(face[0])
						DrawQuad(length/2,width/2,thik,true,false,false);                      // front
					if(face[1])
						DrawQuad(length/2,width/2,-thikness+thik,true,false,false);               // back

					if(face[2])
						DrawQuad(-length/2,width/2,-thikness+thik,false,true,false);              // left
					if(face[3])
						DrawQuad(length/2,width/2,-thikness+thik,false,true,false);               // right

					if(face[4])
						DrawQuad(length/2,-width/2,-thikness+thik,false,false,true);              // down
					if(face[5])
						DrawQuad(length/2,width/2,-thikness+thik,false,false,true);               // up
				glPopMatrix();
